 ...
 *    also gone.
 *  - `owner_id` is a SNAPSHOT of who owned the listing at the moment the
 ...
 *    the owner of record then, which is not necessarily who owns the listing
 *    today. The enquiry itself went to the listing's mailbox, which every
 *    staff member answers. NULL when the listing had no owner account at send
 *    time and its co-managers were answering it
 *    (`MakeListingEnquiryOwnerNullable1821500100000`).
...
export class ListingEnquiry {
  ...
  /** Who owned the listing when this was sent: a fixed snapshot of the owner
   *  of record, which later ownership changes leave alone. NULL when the
   *  listing had no owner account then and its co-managers received the
   *  enquiry. */
